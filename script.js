...
function createMonthlyChart() {
    ...
    document.getElementById("chartTitle").innerText =
        `Consumo de Energia em kWh - ${currentYear}`;

    const labels = monthNames.map((month) => `${month}/${currentYear}`);
    ...
    document.getElementById("chartTotals").innerHTML = `
        <span>Total fechado: <strong>${totalFechado.toLocaleString("pt-BR")}</strong> kWh</span>
        <span>Média Mensal (YTD): <strong>${mediaMensalYtd.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}</strong> kWh</span>
        <span>Meta: <strong>${(150000).toLocaleString("pt-BR")}</strong> kWh</span>
    `;
    ...
    window.chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: "Meta mensal",
                    data: Array(labels.length).fill(150000),
